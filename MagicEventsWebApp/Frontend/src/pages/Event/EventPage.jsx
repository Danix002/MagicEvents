	...
	return !event ? (
		...
		<div className="h-full bg-gradient-to-br from-[#505458] to-[#363540] overflow-hidden flex flex-col sm:flex-row justify-center">
			...
			<div className="h-full bg-gradient-to-br from-[#505458] to-[#363540] overflow-hidden flex">
					...
					<div className="p-4 md:p-6">
						<div className="max-w-4xl mx-auto">
							...
							<div className="bg-white bg-opacity-20 backdrop-blur-sm rounded-2xl p-6 shadow-xl mb-6">
								<h1 className="text-3xl md:text-4xl font-bold text-[#363540] mb-4 w-full break-words overflow-hidden">
									{event.title}
								</h1>
								<div
								...
								<div
									...
									<div className="flex flex-col sm:flex-row sm:items-center gap-2 sm:gap-4">
										<div
											className="bg-[#E4DCEF] text-[#363540] rounded-full px-4 py-2 font-bold text-center">
							...
							<div className="lg:hidden space-y-4 mb-6">
								...
								<details className="bg-white bg-opacity-20 backdrop-blur-sm rounded-2xl shadow-xl">
									<summary
										className="p-4 cursor-pointer text-[#363540] font-semibold flex items-center gap-2">
										...
										<div>
											...
											<div className="space-y-2 max-h-32 overflow-y-auto">
												{event.partecipants.map((p, index) => (
													<div key={index}
														 className="bg-[#363540] rounded-lg p-2 flex items-center gap-2">
														<div
															className="w-6 h-6 bg-[#EE0E51] rounded-full flex items-center justify-center text-white font-bold text-xs">
											...
											<div className="space-y-2">
												{event.admins.map((p, index) => (
													<div key={index}
														 className="bg-[#363540] rounded-lg p-2 flex items-center gap-2">
														<div
															className="w-6 h-6 bg-gradient-to-r from-[#EE0E51] to-[#ff4574] rounded-full flex items-center justify-center text-white font-bold text-xs">
															{p.charAt(0).toUpperCase()}
														</div>
														<span className="text-[#E4DCEF] text-sm">{p}</span>
													</div>
								...
								<details className="bg-white bg-opacity-20 backdrop-blur-sm rounded-2xl shadow-xl">
									<summary
										...
										Servizi
										({eventServices ? Object.values(eventServices).filter(Boolean).length : 0})
									</summary>
											...
											<div className="space-y-2">
												{eventServices.board && (
													<NavLink to={`/${eventId}/board`} className="block">
														<div
															className="bg-[#363540] hover:bg-[#EE0E51] transition-colors rounded-lg p-3 flex items-center justify-between">
															<div className="flex items-center gap-3">
																<FontAwesomeIcon icon={faMessage}
																				 className="text-[#EE0E51] text-sm"/>
																<span className="text-[#E4DCEF] text-sm">Bacheca</span>
															</div>
															<FontAwesomeIcon icon={faArrowRight}
																			 className="text-[#E4DCEF] text-sm"/>
														</div>
													</NavLink>
												)}
							...
							{popularImages.length > 0 && (
								<div className="bg-white bg-opacity-20 backdrop-blur-sm rounded-2xl p-6 shadow-xl mb-6">
									<div className="flex items-center justify-between mb-6">
										<div className="flex items-center gap-3">
											<div
												className="w-8 h-8 bg-gradient-to-r from-[#EE0E51] to-[#FF6B9D] rounded-full flex items-center justify-center">
												<FontAwesomeIcon icon={faHeart} className="text-white text-sm"/>
											</div>
											<h2 className="text-xl sm:text-2xl font-semibold text-[#363540]">Immagini
												Popolari</h2>
										</div>
										{eventServices?.gallery && (
				...
				<div className="hidden lg:block w-150 bg-[#363540] border-l border-[#E4DCEF]/20 overflow-y-auto order-2">
					<div className="p-6">
						...
						<div className="mb-8">
							...
							<div className="space-y-4">
								<div>
									<h4 className="text-sm font-medium text-[#E4DCEF] mb-2">Partecipanti
										({event.partecipants.length})</h4>
									<div className="max-h-32 overflow-y-auto space-y-2">
										{event.partecipants.map((p, index) => (
											<div key={index}
												 className="bg-[#505458] rounded-lg p-2 flex items-center gap-2">
												<div
													className="w-6 h-6 bg-[#EE0E51] rounded-full flex items-center justify-center text-white font-bold text-xs">
													{p.charAt(0).toUpperCase()}
												</div>
								...
								<div>
									<h4 className="text-sm font-medium text-[#E4DCEF] mb-2">Amministratori
										({event.admins.length})</h4>
									<div className="max-h-32 overflow-y-auto space-y-2">
										{event.admins.map((p, index) => (
											<div key={index}
												 className="bg-[#505458] rounded-lg p-2 flex items-center gap-2">
							...
							{eventServices ? (
								<div className="space-y-2">
									{eventServices.board && (
										<NavLink to={`/${eventId}/board`} className="block">
											<div
												className="bg-[#505458] hover:bg-[#EE0E51] transition-colors rounded-lg p-3 flex items-center justify-between group">
												<div className="flex items-center gap-3">
													<FontAwesomeIcon icon={faMessage}
																	 className="text-[#EE0E51] group-hover:text-white text-sm"/>
													<span
														className="text-[#E4DCEF] group-hover:text-white text-sm">Bacheca</span>
												</div>
												<FontAwesomeIcon icon={faArrowRight}
																 className="text-[#E4DCEF] group-hover:text-white text-sm"/>
											</div>
										</NavLink>
									)}
									{eventServices.gallery && (
											...
											<div
												...
												<div className="flex items-center gap-3">
													<FontAwesomeIcon icon={faImage}
																	 className="text-[#EE0E51] group-hover:text-white text-sm"/>
													<span
														className="text-[#E4DCEF] group-hover:text-white text-sm">Galleria</span>
												</div>
												<FontAwesomeIcon icon={faArrowRight}
																 className="text-[#E4DCEF] group-hover:text-white text-sm"/>
											</div>
								...
								<div className="text-center py-4">
									<FontAwesomeIcon icon={faClipboard}
													 className="text-2xl text-[#E4DCEF] opacity-50 mb-2"/>
